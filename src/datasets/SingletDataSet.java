...
public class SingletDataSet extends AbstractDataSet {
	...
	public SingletDataSet(String SPROXFile, String DenaturantFile,
			TextFlow output, boolean detectOx) {
		super(SPROXFile, DenaturantFile, output, detectOx);
	}

	/**
	 * Takes the file defined in the constructor and digests it into individual
	 * runs along with a header
	 */
	public String load() {
		/*
		...
		try (BufferedReader br = new BufferedReader(new FileReader(
				super.getSPROX1File()))) {
			String line;
			// adds extra leading infomation to a separate list
	...
	protected Void call() throws Exception {
		...
		/*
		 * runs = List of String arrays, one array per run 0 = Peptide 1 =
		 * Protein 2 = Integral Sum 3 = Retention Time 4.. 4 + Denats.length =
		 * intensities at corresponding denaturant
		 */

		/*
		 * DenaturantConcentrations = Double array of included denatuant
		 * concentrations
		 */

		// update the last row in headers to account for new additions, chalf,
		// chalfSD, b, bSD, adjRSquared
		List<String> headerList = FFOperations.asList(this.headers1
				.get(this.headers1.size() - 1));
		...
		/*
		 * For each run in runs, calculate chalf and b. Update the run in runs
		...
		for (int i = 0; i < runs1.size(); i++) {
			...
			TextFlowWriter.writeInfo(
					"Calculating #" + (i + 1) + " / " + runs1.size(),
					super.output);
			try {
				if (run[0].length() > 0) {
					SingletPeptideContainer pc = new SingletPeptideContainer(
							run, super.getDenaturants(), LEADING_IDENTIFIERS);

					DataRun r = new DataRun(pc.intensities,
							this.DenaturantConcentrations, super.getDetectOx());
	...
	public class SingletPeptideContainer {
		...
		public Chartable toChartable() {
			return new Chartable(this.peptide, this.protein, "-", this.intsum,
					this.rt, this.intensities, this.denaturants, this.chalf,
					this.chalfSD, this.b, this.bSD, this.adjRSq,
					this.indexRemoved, this.A, this.B);
		}
