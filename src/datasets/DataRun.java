...
public class DataRun extends Task<SingleFit> {
	...
	private double[] calculateFit(Double[] intensities, Double[] denaturants) {
		...
		estimates[1] = FFConstants.InitialBValue; // b, calculated using
													// heuristics
	...
	protected SingleFit call() throws Exception {
		...
		double[] calculatedFit = this.calculateFit(y, x);
		fitList.add(new SingleFit(calculatedFit, -1, AAndB[0], AAndB[1]));
		...
		for (int i = 0; i < this.intensities.length; i++) {
			...
			for (int j = 0; j < this.intensities.length; j++) {
				if (j == i)
					continue; // ignore the one we want to remove
			...
			// calculate fit
			double[] calculatedFitIndexRemoved = this.calculateFit(
					newIntensities, newDenaturants);
			SingleFit fit = new SingleFit(calculatedFitIndexRemoved, i, A, B);
			fitList.add(fit);
		}
	...
	private double[] calculateAAndB(Double[] intensities2) {
		 ...
		 * 
		 * Assume to be Non-Oxidized if first half of values average to be
		 * greater than the second half of values
		 */
